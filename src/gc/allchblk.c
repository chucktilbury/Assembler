...
GC_API void GC_CALL GC_iterate_free_hblks(GC_walk_free_blk_fn fn, GC_word client_data) {
    int i;
...
static void GC_CALLBACK print_hblkfreelist_item(struct hblk* h, int i, GC_word prev_index_ptr) {
    hdr* hhdr = HDR(h);
    ...
    for(i = 0; i < GC_n_heap_sects; ++i) {
        ...
        for(p = start; (word)p < (word)end;) {
            ...
            if(HBLK_IS_FREE(hhdr)) {
                int correct_index = GC_hblk_fl_from_blocks(divHBLKSZ(hhdr->hb_sz));
                int actual_index;

                GC_printf("\t%p\tfree block of size 0x%lx bytes%s\n", (void*)p,
                          (unsigned long)(hhdr->hb_sz), IS_MAPPED(hhdr) ? "" : " (unmapped)");
...
static GC_bool
setup_header(hdr* hhdr, struct hblk* block, size_t byte_sz, int kind, unsigned flags) {
    word descr;
 ...
 */
STATIC void GC_split_block(struct hblk* h, hdr* hhdr, struct hblk* n, hdr* nhdr, int index /* Index of free list */) {
...
STATIC struct hblk*
GC_allochblk_nth(size_t sz /* bytes */, int kind, unsigned flags, int n, int may_split);
...
GC_allochblk(size_t sz, int kind, unsigned flags /* IGNORE_OFF_PAGE or 0 */) {
    ...
    may_split = TRUE;
    if(GC_use_entire_heap || GC_dont_gc || GC_heapsize - GC_large_free_bytes < GC_requested_heapsize ||
...
STATIC struct hblk* GC_allochblk_nth(size_t sz, int kind, unsigned flags, int n, int may_split) {
    ...
    for(hbp = GC_hblkfreelist[n];; hbp = hhdr->hb_next) {
        ...
        if(!IS_UNCOLLECTABLE(kind) &&
           (kind != PTRFREE || size_needed > (signed_word)MAX_BLACK_LIST_ALLOC)) {
            struct hblk* lasthbp = hbp;
            ptr_t search_end = (ptr_t)hbp + size_avail - size_needed;
            signed_word orig_avail = size_avail;
            signed_word eff_size_needed =
            (flags & IGNORE_OFF_PAGE) != 0 ? (signed_word)HBLKSIZE : size_needed;

            while((word)lasthbp <= (word)search_end &&
                  (thishbp = GC_is_black_listed(lasthbp, (word)eff_size_needed)) != 0) {
                lasthbp = thishbp;
            }
            ...
            else if(size_avail == 0 && size_needed == (signed_word)HBLKSIZE &&
                    IS_MAPPED(hhdr)) {
                if(!GC_find_leak) {
...
GC_INNER void GC_freehblk(struct hblk* hbp) {
    ...
    if(prev /* != NULL */) { /* CPPCHECK */
        prevhdr = HDR(prev);
        if(IS_MAPPED(prevhdr) && (signed_word)(hhdr->hb_sz + prevhdr->hb_sz) > 0) {
