    ...
    for(;;) {
        ...
        if(0 == avail || (yavail = CORD_pos_chars_left(ypos)) == 0) {
            ...
            if(avail > yavail)
                avail = yavail;
            result =
            strncmp(CORD_pos_cur_char_addr(xpos), CORD_pos_cur_char_addr(ypos), avail);
            if(result != 0)
                return result;
...
size_t CORD_str(CORD x, size_t start, CORD s) {
    ...
    for(match_pos = start;; match_pos++) {
        if((x_buf & mask) == s_buf &&
           (slen == start_len ||
            CORD_ncmp(x, match_pos + start_len, s, start_len, slen - start_len) == 0))
            break;
...
static void* GC_CALLBACK refill_cache(void* client_data) {
    ...
    GC_END_STUBBORN_CHANGE((/* no volatile */ void*)(state->lf_cache + line_no));
