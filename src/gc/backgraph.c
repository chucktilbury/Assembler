...
static void push_in_progress(ptr_t p) {
    ...
    if(n_in_progress >= in_progress_size) {
        ...
        if(NULL == in_progress_space) {
            in_progress_size =
            ROUNDUP_PAGESIZE_IF_MMAP(INITIAL_IN_PROGRESS * sizeof(ptr_t)) / sizeof(ptr_t);
            new_in_progress_space = (ptr_t*)GET_MEM(in_progress_size * sizeof(ptr_t));
        }
        else {
            in_progress_size *= 2;
            new_in_progress_space = (ptr_t*)GET_MEM(in_progress_size * sizeof(ptr_t));
...
#ifndef GWW_VDB
        GC_scratch_recycle_no_gww(in_progress_space, n_in_progress * sizeof(ptr_t));
#elif defined(LINT2)
