...
#if(defined(DYNAMIC_LOADING) || defined(MSWIN32) || defined(MSWINCE) || defined(CYGWIN32)) && \
!defined(PCR)
...
!(defined(ALPHA) && defined(OSF1)) && !(defined(FREEBSD) && defined(__ELF__)) &&   \
!(defined(LINUX) && defined(__ELF__)) && !(defined(NETBSD) && defined(__ELF__)) && \
!(defined(OPENBSD) && (defined(__ELF__) || defined(M68K))) &&                      \
!defined(HAIKU) && !defined(HURD) && !defined(NACL) && !defined(CPPCHECK)
...
STATIC void GC_register_map_entries(const char* maps) {
    ...
    greatest_ha =
    GC_our_memory[GC_n_memory - 1].hs_start + GC_our_memory[GC_n_memory - 1].hs_bytes;
    ...
    for(;;) {
        ...
        if(prot[1] == 'w') {
            ...
            while(i < GC_n_memory && (word)GC_our_memory[i].hs_start < (word)end &&
                  (word)start < (word)end) {
...
STATIC int GC_register_dynlib_callback(struct dl_phdr_info* info, size_t size, void* ptr) {
    const ElfW(Phdr) * p;
    ptr_t start, end;
    int i;
                ...
                if((word)start >= (word)load_segs[j].start &&
                   (word)start < (word)load_segs[j].end) {
                    if(load_segs[j].start2 != 0) {
                        ...
                        GC_ASSERT((word)end <= (((word)load_segs[j].end + GC_page_size - 1) &
                                                ~(GC_page_size - 1)));
...
STATIC GC_bool GC_register_dynamic_libraries_dl_iterate_phdr(void) {
    ...
    if(did_something) {
        ...
        if(NULL == *(char* volatile*)&datastart || (word)datastart > (word)dataend)
            ABORT_ARG2("Wrong DATASTART/END pair", ": %p .. %p",
                       (void*)datastart, (void*)dataend);
        ...
        if((word)DATASTART2 - 1U >= (word)DATAEND2) {
            ...
            ABORT_ARG2("Wrong DATASTART/END2 pair", ": %p .. %p",
                       (void*)DATASTART2, (void*)DATAEND2);
        }
...
STATIC void GC_cond_add_roots(char* base, char* limit) {
    ...
    for(;;) {
        ...
        if((word)next_stack_lo >= (word)limit)
            break;
...
#else
    char* stack_top =
    (char*)((word)GC_approx_sp() & ~(word)(GC_sysinfo.dwAllocationGranularity - 1));
...
GC_INNER void GC_register_dynamic_libraries(void) {
    ...
    while((word)p < (word)GC_sysinfo.lpMaximumApplicationAddress) {
        ...
        if((word)p > (word)new_limit /* overflow */)
            break;
        p = (LPVOID)new_limit;
    }
    GC_cond_add_roots(base, limit);
...
STATIC const char* const GC_dyld_add_sect_fmts[] = { "__bss%u", "__pu_bss%u",
                                                     "__zo_bss%u", "__zo_pu_bss%u" };
...
STATIC void GC_dyld_image_add(const struct GC_MACH_HEADER* hdr, intptr_t slide) {
    ...
    for(i = 0; i < sizeof(GC_dyld_sections) / sizeof(GC_dyld_sections[0]); i++) {
        sec = GC_GETSECTBYNAME(hdr, GC_dyld_sections[i].seg, GC_dyld_sections[i].sect);
...
STATIC void GC_dyld_image_remove(const struct GC_MACH_HEADER* hdr, intptr_t slide) {
    unsigned long start, end;
    unsigned i, j;
    const struct GC_MACH_SECTION* sec;
#if defined(DARWIN_DEBUG) && !defined(NO_DEBUGGING)
...
#ifdef DARWIN_DEBUG
        GC_log_printf("Removing section __DATA,%s at %p-%p (%lu bytes) from "
                      "image %s\n",
                      GC_dyld_sections[i].sect, (void*)start, (void*)end,
    ...
    for(; p != NIL; p = p->lf_prev) {
        for(q = p->lf_ls; q != NIL; q = q->ls_next) {
            if((q->ls_flags & PCR_IL_SegFlags_Traced_MASK) == PCR_IL_SegFlags_Traced_on) {
                GC_add_roots_inner((ptr_t)q->ls_addr, (ptr_t)q->ls_addr + q->ls_bytes, TRUE);
            }
        }
    }
}
...
GC_API void GC_CALL GC_register_has_static_roots_callback(GC_has_static_roots_func callback) {
