...
STATIC void GC_grow_table(struct hash_chain_entry*** table,
                          unsigned* log_size_ptr,
                          const word* entries_ptr) {
    word i;
...
STATIC int GC_register_disappearing_link_inner(struct dl_hashtbl_s* dl_hashtbl,
                                               void** link,
                                               const void* obj,
    ...
    if(EXPECT(NULL == dl_hashtbl->head, FALSE) ||
       EXPECT(dl_hashtbl->entries > ((word)1 << dl_hashtbl->log_size), FALSE)) {
    ...
    new_dl =
    (struct disappearing_link*)GC_INTERNAL_MALLOC(sizeof(struct disappearing_link), NORMAL);
...
GC_unregister_disappearing_link_inner(struct dl_hashtbl_s* dl_hashtbl, void** link) {
    ...
    index = HASH2(link, dl_hashtbl->log_size);
    for(curr_dl = dl_hashtbl->head[index]; curr_dl; curr_dl = dl_next(curr_dl)) {
        if(curr_dl->dl_hidden_link == GC_HIDE_POINTER(link)) {
    ...
    for(i = 0; i < GC_toggleref_array_size; ++i) {
        ...
        switch(GC_toggleref_callback(obj)) {
            ...
            case GC_TOGGLE_REF_WEAK:
                ...
                break;
            default: ABORT("Bad toggle-ref status returned by callback");
        }
    }
...
static GC_bool ensure_toggleref_capacity(size_t capacity_inc) {
    ...
    if(GC_toggleref_array_size + capacity_inc >= GC_toggleref_array_capacity) {
        ...
        if(EXPECT(GC_toggleref_array_size > 0, TRUE))
            BCOPY(GC_toggleref_arr, new_array, GC_toggleref_array_size * sizeof(GCToggleRef));
...
GC_API int GC_CALL GC_register_long_link(void** link, const void* obj) {
    ...
    return GC_register_disappearing_link_inner(&GC_ll_hashtbl, link, obj, "long dl");
}
...
STATIC int GC_move_disappearing_link_inner(struct dl_hashtbl_s* dl_hashtbl,
                                           void** link,
                                           void** new_link) {
    ...
    for(curr_dl = dl_hashtbl->head[curr_index]; curr_dl; curr_dl = dl_next(curr_dl)) {
    ...
    for(new_dl = dl_hashtbl->head[new_index]; new_dl; new_dl = dl_next(new_dl)) {
...
STATIC void GC_normal_finalize_mark_proc(ptr_t p) {
    GC_mark_stack_top =
    GC_push_obj(p, HDR(p), GC_mark_stack_top, GC_mark_stack + GC_mark_stack_size);
}
...
STATIC void GC_ignore_self_finalize_mark_proc(ptr_t p) {
    ...
    for(current_p = p; (word)current_p <= (word)scan_limit; current_p += ALIGNMENT) {
        word q;
...
STATIC void GC_register_finalizer_inner(void* obj,
    ...
    for(;;) {
        ...
        new_fo = (struct finalizable_object*)GC_INTERNAL_MALLOC(sizeof(struct finalizable_object),
                                                                NORMAL);
...
#ifndef NO_DEBUGGING
STATIC void GC_dump_finalization_links(const struct dl_hashtbl_s* dl_hashtbl) {
    ...
    for(i = 0; i < dl_size; i++) {
        ...
        for(curr_dl = dl_hashtbl->head[i]; curr_dl != 0; curr_dl = dl_next(curr_dl)) {
...
GC_INLINE void GC_make_disappearing_links_disappear(struct dl_hashtbl_s* dl_hashtbl,
    ...
    for(i = 0; i < dl_size; i++) {
        ...
        for(curr_dl = dl_hashtbl->head[i]; curr_dl != NULL; curr_dl = next_dl) {
            ...
            if(is_remove_dangling) {
                ...
                if(EXPECT(GC_is_marked((ptr_t)GC_REVEAL_POINTER(curr_dl->dl_hidden_obj)), TRUE)) {
                    prev_dl = curr_dl;
                    continue;
                }
            ...
            while(curr_fo != NULL) {
                ...
                if(curr_fo->fo_mark_proc == GC_unreachable_finalize_mark_proc) {
                    ...
                    if(!GC_is_marked(real_ptr)) {
                        ...
                        curr_fo->fo_hidden_base = GC_HIDE_POINTER(curr_fo->fo_hidden_base);
                        GC_bytes_finalized -=
                        curr_fo->fo_object_size + sizeof(struct finalizable_object);
...
GC_API int GC_CALL GC_invoke_finalizers(void) {
    ...
    while(GC_should_invoke_finalizers()) {
        ...
        (*(curr_fo->fo_fn))((ptr_t)(curr_fo->fo_hidden_base), curr_fo->fo_client_data);
...
GC_INNER void GC_print_finalization_stats(void) {
    ...
    (long)IF_LONG_REFS_PRESENT_ELSE(GC_old_ll_entries - GC_ll_hashtbl.entries, 0));
}
#endif /* !SMALL_CONFIG */
