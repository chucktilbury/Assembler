...
#if defined(__GLIBC__) || defined(SOLARIS) || defined(HPUX) || \
defined(IRIX5) || defined(OSF1)
...
#ifdef PARALLEL_MARK
        AO_store((volatile AO_t*)&((oh*)dest)->oh_back_ptr, (AO_t)HIDE_BACK_PTR(source));
#else
...
GC_API GC_ref_kind GC_CALL GC_get_back_ptr_info(void* dest, void** base_p, size_t* offset_p) {
...
GC_INNER void* GC_store_debug_info_inner(void* p, word sz, const char* string, int linenum) {
...
static void* store_debug_info(void* p, size_t lb, const char* fn, GC_EXTRA_PARAMS) {
    void* result;
    DCL_LOCK_STATE;
...
GC_API void GC_CALL GC_register_describe_type_fn(int kind, GC_describe_type_fn fn) {
...
STATIC void GC_print_obj(ptr_t p) {
    ...
    if(0 != GC_describe_type_fns[kind] && GC_is_marked(ohdr)) {
        ...
        switch(kind) {
            case PTRFREE: kind_str = "PTRFREE"; break;
            case NORMAL: kind_str = "NORMAL"; break;
            case UNCOLLECTABLE: kind_str = "UNCOLLECTABLE"; break;
#ifdef GC_ATOMIC_UNCOLLECTABLE
            case AUNCOLLECTABLE: kind_str = "ATOMIC_UNCOLLECTABLE"; break;
#endif
            default:
                kind_str = NULL;
...
/* clobbered_addr.                                                    */
STATIC void GC_print_smashed_obj(const char* msg, void* p, ptr_t clobbered_addr) {
    oh* ohdr = (oh*)GC_base(p);
    ...
    if((word)clobbered_addr <= (word)(&ohdr->oh_sz) || ohdr->oh_string == 0) {
        GC_err_printf("%s %p in or near object at %p(<smashed>, appr. sz= "
                      "%lu)\n",
...
GC_API GC_ATTR_MALLOC void* GC_CALL GC_debug_malloc(size_t lb, GC_EXTRA_PARAMS) {
    void* result;
    ...
    result =
    GC_malloc(lb < GC_SIZE_MAX - DEBUG_BYTES ? lb + DEBUG_BYTES : GC_SIZE_MAX >> 1);
#else
...
GC_API GC_ATTR_MALLOC void* GC_CALL GC_debug_malloc_ignore_off_page(size_t lb, GC_EXTRA_PARAMS) {
    void* result = GC_malloc_ignore_off_page(SIZET_SAT_ADD(lb, DEBUG_BYTES));

    return store_debug_info(result, lb, "GC_debug_malloc_ignore_off_page", OPT_RA s, i);
}

GC_API GC_ATTR_MALLOC void* GC_CALL GC_debug_malloc_atomic_ignore_off_page(size_t lb, GC_EXTRA_PARAMS) {
    void* result = GC_malloc_atomic_ignore_off_page(SIZET_SAT_ADD(lb, DEBUG_BYTES));

    return store_debug_info(result, lb, "GC_debug_malloc_atomic_ignore_off_page",
                            OPT_RA s, i);
}
...
STATIC void* GC_debug_generic_malloc(size_t lb, int knd, GC_EXTRA_PARAMS) {
    ...
    return store_debug_info(result, lb, "GC_debug_generic_malloc", OPT_RA s, i);
}
...
GC_INNER void* GC_debug_generic_malloc_inner_ignore_off_page(size_t lb, int k) {
    ...
    GC_ASSERT(I_HOLD_LOCK());
    result = GC_generic_malloc_inner_ignore_off_page(SIZET_SAT_ADD(lb, DEBUG_BYTES), k);
    if(NULL == result) {
        GC_err_printf("GC internal allocation (%lu bytes) returning NULL\n",
                      (unsigned long)lb);
        return NULL;
...
GC_API GC_ATTR_MALLOC void* GC_CALL GC_debug_malloc_atomic(size_t lb, GC_EXTRA_PARAMS) {
    void* result = GC_malloc_atomic(SIZET_SAT_ADD(lb, DEBUG_BYTES));

    return store_debug_info(result, lb, "GC_debug_malloc_atomic", OPT_RA s, i);
}

GC_API GC_ATTR_MALLOC char* GC_CALL GC_debug_strdup(const char* str, GC_EXTRA_PARAMS) {
    char* copy;
    size_t lb;
    if(str == NULL) {
        if(GC_find_leak)
...
GC_API GC_ATTR_MALLOC char* GC_CALL GC_debug_strndup(const char* str, size_t size, GC_EXTRA_PARAMS) {
    char* copy;
...
GC_API GC_ATTR_MALLOC wchar_t* GC_CALL GC_debug_wcsdup(const wchar_t* str, GC_EXTRA_PARAMS) {
...
GC_API GC_ATTR_MALLOC void* GC_CALL GC_debug_malloc_uncollectable(size_t lb, GC_EXTRA_PARAMS) {
    void* result = GC_malloc_uncollectable(SIZET_SAT_ADD(lb, UNCOLLECTABLE_DEBUG_BYTES));

    return store_debug_info(result, lb, "GC_debug_malloc_uncollectable", OPT_RA s, i);
}

#ifdef GC_ATOMIC_UNCOLLECTABLE
GC_API GC_ATTR_MALLOC void* GC_CALL GC_debug_malloc_atomic_uncollectable(size_t lb, GC_EXTRA_PARAMS) {
    void* result =
    GC_malloc_atomic_uncollectable(SIZET_SAT_ADD(lb, UNCOLLECTABLE_DEBUG_BYTES));

    return store_debug_info(result, lb, "GC_debug_malloc_atomic_uncollectable",
                            OPT_RA s, i);
}
...
((defined(NEED_CALLINFO) && defined(GC_HAVE_BUILTIN_BACKTRACE)) || \
 defined(GC_LINUX_THREADS) || defined(GC_SOLARIS_THREADS) || defined(MSWIN32))
    ...
    if((ptr_t)p - (ptr_t)base != sizeof(oh)) {
        ...
        if(clobbered != 0) {
            ...
            if(((oh*)base)->oh_sz == sz) {
                GC_print_smashed_obj("GC_debug_free: found previously "
                                     "deallocated (?) object at",
                                     p, clobbered);
                return; /* ignore double free */
            }
            else {
                GC_print_smashed_obj("GC_debug_free: found smashed location at", p, clobbered);
            }
        }
...
GC_API void* GC_CALL GC_debug_realloc(void* p, size_t lb, GC_EXTRA_PARAMS) {
    ...
    switch(hhdr->hb_obj_kind) {
        case NORMAL: result = GC_debug_malloc(lb, OPT_RA s, i); break;
        case PTRFREE: result = GC_debug_malloc_atomic(lb, OPT_RA s, i); break;
        case UNCOLLECTABLE:
...
GC_API GC_ATTR_MALLOC void* GC_CALL GC_debug_generic_or_special_malloc(size_t lb, int knd, GC_EXTRA_PARAMS) {
    switch(knd) {
        case PTRFREE: return GC_debug_malloc_atomic(lb, OPT_RA s, i);
        case NORMAL: return GC_debug_malloc(lb, OPT_RA s, i);
        case UNCOLLECTABLE:
...
#endif
        default: return GC_debug_generic_malloc(lb, knd, OPT_RA s, i);
    }
}
...
static void store_old(void* obj,
...
GC_API void GC_CALL GC_debug_register_finalizer(void* obj,
    ...
    if(0 == fn) {
        ...
        GC_register_finalizer(base, GC_debug_invoke_finalizer, cd, &my_old_fn, &my_old_cd);
    }
