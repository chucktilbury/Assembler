...
#define GC_FAST_MALLOC_GRANS(result, granules, tiny_fl, num_direct, kind, default_expr, init)                           \
    do {                                                                                                                \
            ...
            for(;;) {                                                                                                   \
                ...
                if((GC_signed_word)my_entry - (GC_signed_word)(num_direct) <= 0 /* (GC_word)my_entry <= (num_direct) */ \
                   && my_entry != 0 /* NULL */) {                                                                       \
                    ...
                    GC_generic_malloc_many(((granules) == 0 ?                                                           \
                                            GC_GRANULE_BYTES :                                                          \
...
#define GC_MALLOC_WORDS_KIND(result, n, tiny_fl, kind, init)                          \
    do {                                                                              \
        ...
        GC_FAST_MALLOC_GRANS(result, granules, tiny_fl, 0, kind,                      \
                             GC_malloc_kind(granules* GC_GRANULE_BYTES, kind), init); \
    } while(0)

#define GC_MALLOC_WORDS(result, n, tiny_fl) \
    GC_MALLOC_WORDS_KIND(result, n, tiny_fl, GC_I_NORMAL, *(void**)(result) = 0)
