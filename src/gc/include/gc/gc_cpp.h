...
#ifdef GC_PLACEMENT_DELETE
inline void
operator delete(void*, GC_NS_QUALIFY(GCPlacement), GC_NS_QUALIFY(GCCleanUpFunc), void*)GC_NOEXCEPT;
#endif

#ifndef GC_NO_INLINE_STD_NEW

#if defined(_MSC_VER) || defined(__DMC__) ||                               \
((defined(__BORLANDC__) || defined(__CYGWIN__) || defined(__CYGWIN32__) || \
...
#else
inline void*
operator new(size_t size, int /* nBlockUse */, const char* /* szFileName */, int /* nLine */) {
    void* obj = GC_malloc_uncollectable(size);
    GC_OP_NEW_OOM_CHECK(obj);
    return obj;
}
...
inline void* gc::operator new(size_t size, GCPlacement gcp) {
    void* obj;
    switch(gcp) {
        case UseGC: obj = GC_MALLOC(size); break;
        case PointerFreeGC: obj = GC_MALLOC_ATOMIC(size); break;
#ifdef GC_ATOMIC_UNCOLLECTABLE
        case PointerFreeNoGC: obj = GC_MALLOC_ATOMIC_UNCOLLECTABLE(size); break;
#endif
        case NoGC:
        default: obj = GC_MALLOC_UNCOLLECTABLE(size);
    }
    GC_OP_NEW_OOM_CHECK(obj);
    return obj;
}
...
inline void GC_CALLBACK gc_cleanup::cleanup(void* obj, void* displ) {
    reinterpret_cast<gc_cleanup*>(reinterpret_cast<char*>(obj) +
                                  reinterpret_cast<ptrdiff_t>(displ))
    ->~gc_cleanup();
}
...
inline void* operator new(size_t size,
    ...
    void* obj;
    switch(gcp) {
        case GC_NS_QUALIFY(UseGC): obj = GC_MALLOC(size);
#ifndef GC_NO_FINALIZATION
            if(cleanup != 0 && obj != 0) {
...
#endif
            break;
        case GC_NS_QUALIFY(PointerFreeGC): obj = GC_MALLOC_ATOMIC(size); break;
#ifdef GC_ATOMIC_UNCOLLECTABLE
