...
!defined(_GNU_SOURCE) && defined(GC_PTHREADS) && !defined(GC_NO_PTHREAD_SIGMASK)
...
GC_INLINE mse* GC_push_obj(ptr_t obj, hdr* hhdr, mse* mark_stack_top, mse* mark_stack_limit) {
...
#define PUSH_CONTENTS(current, mark_stack_top, mark_stack_limit, source)                 \
    do {                                                                                 \
        ...
        mark_stack_top = GC_push_contents_hdr(current, mark_stack_top, mark_stack_limit, \
                                              source, my_hhdr, TRUE);                    \
    } while(0)
...
#define SET_MARK_BIT_EXIT_IF_SET(hhdr, bit_no)                                                    \
    { /* cannot use do-while(0) here */                                                           \
        ...
        OR_WORD_EXIT_IF_SET(mark_word_addr, (word)1 << modWORDSZ(bit_no)); /* contains "break" */ \
    }
...
GC_INLINE mse* GC_push_contents_hdr(ptr_t current,
                                    mse* mark_stack_top,
    ...
    do {
        ...
        TRACE_TARGET(base,
                     GC_log_printf("GC #%lu: marking %p from %p instead\n",
