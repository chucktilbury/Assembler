...
(GC_GNUC_PREREQ(3, 0) && defined(__STRICT_ANSI__)) || defined(__BORLANDC__) || \
defined(__WATCOMC__)
...
#define MS_TIME_DIFF(a, b)                                                                       \
    ((unsigned long)((long)(a.tv_sec - b.tv_sec) * 1000 + (long)(a.tv_usec - b.tv_usec) / 1000 - \
                     (a.tv_usec < b.tv_usec && (long)(a.tv_usec - b.tv_usec) % 1000 != 0 ? 1 : 0)))
/* "a" time is expected to be not earlier than  */
/* "b" one; the result has unsigned long type.  */
#define NS_FRAC_TIME_DIFF(a, b)                                                                         \
    ((unsigned long)((a.tv_usec < b.tv_usec && (long)(a.tv_usec - b.tv_usec) % 1000 != 0 ? 1000L : 0) + \
...
#define START_WORLD() \
    PCR_ThCtl_SetExclusiveMode(PCR_ThCtl_ExclusiveMode_null, PCR_allSigsBlocked, PCR_waitForever)
#else
...
#define WARN(msg, arg) \
    (*GC_current_warn_proc)((/* no const */ char*)("GC Warning: " msg), (word)(arg))
...
#define set_pht_entry_from_index_concurrent(bl, index) \
    AO_or((volatile AO_t*)&(bl)[divWORDSZ(index)], (AO_t)((word)1 << modWORDSZ(index)))
#else
#define set_pht_entry_from_index_concurrent(bl, index) \
...
(defined(CYGWIN32) || defined(MSWIN32) || defined(MSWINCE) || defined(USE_PROC_FOR_LIBRARIES))
#ifdef LARGE_CONFIG
#if CPP_WORDSZ > 32
...
GC_INNER void GC_push_all_register_sections(ptr_t bs_lo,
                                            ptr_t bs_hi,
                                            int eager,
...
#define FINAL_MARK_BIT(sz) \
    ((sz) > MAXOBJBYTES ? MARK_BITS_PER_HBLK : BYTES_TO_GRANULES((sz)*HBLK_OBJS(sz)))
#endif
...
#define GC_PUSH_ALL_SYM(sym) \
    GC_push_all((/* no volatile */ void*)&(sym), (/* no volatile */ void*)(&(sym) + 1))
...
#define GC_push_conditional_static(b, t, all) ((void)(all), GC_push_all(b, t))
#else
...
#define GET_PROCEDURE_STACK_LOCAL(pbuf, psz)                                  \
    do {                                                                      \
        ...
        if(capacity > ALLOCA_SAFE_LIMIT &&                                    \
           EXPECT(((capacity - *(psz)) & ~(GC_page_size - 1)) != 0, FALSE)) { \
...
#define FREE_PROCEDURE_STACK_LOCAL(buf, sz) \
    (void)((sz) > ALLOCA_SAFE_LIMIT ? (GC_unmap_procedure_stack_buf(buf, sz), 0) : 0)
...
GC_INNER ptr_t GC_build_fl(struct hblk* h, size_t words, GC_bool clear, ptr_t list);
...
GC_INNER struct hblk* GC_allochblk(size_t size_in_bytes, int kind, unsigned flags);
...
GC_INNER ptr_t GC_reclaim_generic(struct hblk* hbp,
                                  hdr* hhdr,
                                  size_t sz,
                                  GC_bool init,
                                  ptr_t list,
...
#else
#define GC_add_to_our_memory(p, bytes) (GC_our_mem_bytes += (bytes), (void)(p))
#endif
...
GC_INNER void GC_remove_protection(struct hblk* h, word nblocks, GC_bool pointerfree);
...
GC_API_PRIV void GC_err_printf(const char* format, ...) GC_ATTR_FORMAT_PRINTF(1, 2);

/* Basic logging routine.  Typically, GC_log_printf is called directly  */
/* only inside various DEBUG_x blocks.                                  */
GC_API_PRIV void GC_log_printf(const char* format, ...) GC_ATTR_FORMAT_PRINTF(1, 2);
...
#define GC_INFOLOG_PRINTF \
    if(GC_quiet) {        \
        ...
        GC_info_log_printf
#endif
GC_INNER void GC_info_log_printf(const char* format, ...) GC_ATTR_FORMAT_PRINTF(1, 2);
GC_INNER void GC_verbose_log_printf(const char* format, ...) GC_ATTR_FORMAT_PRINTF(1, 2);
...
GC_INNER void* GC_store_debug_info_inner(void* p, word sz, const char* str, int linenum);
