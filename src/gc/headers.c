...
GC_INNER struct hblkhdr* GC_install_header(struct hblk* h) {
    ...
    GC_ASSERT(I_HOLD_LOCK());
    if(EXPECT(!get_index((word)h), FALSE))
        return NULL;
...
GC_API void GC_CALL GC_apply_to_all_blocks(GC_walk_hblk_fn fn, GC_word client_data) {
    signed_word j;
    bottom_index* index_p;

    for(index_p = GC_all_bottom_indices; index_p != 0; index_p = index_p->asc_link) {
