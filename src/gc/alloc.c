...
((GC_VERSION_MAJOR << 16) | (GC_VERSION_MINOR << 8) | GC_VERSION_MICRO);
#endif

GC_API unsigned GC_CALL GC_get_version(void) {
    return (GC_VERSION_MAJOR << 16) | (GC_VERSION_MINOR << 8) | GC_VERSION_MICRO;
}
...
STATIC word GC_adj_bytes_allocd(void) {
    ...
    (signed_word)GC_bytes_freed + (signed_word)GC_finalizer_bytes_freed - expl_managed;
...
STATIC void GC_maybe_gc(void) {
    ...
    if(GC_need_full_gc || n_partial_gcs >= GC_full_freq) {
        GC_COND_LOG_PRINTF("***>Full mark for collection #%lu after %lu allocd "
                           "bytes\n",
    ...
    if(GC_incremental && GC_collection_in_progress()) {
        ...
        if(i < max_deficit) {
            ...
            if(GC_time_limit != GC_TIME_UNLIMITED && GC_n_attempts < max_prior_attempts)
                GET_TIME(GC_start_time);
#endif
            if(GC_stopped_mark(GC_n_attempts < max_prior_attempts ? GC_timeout_stop_func :
                                                                    GC_never_stop_func)) {
                GC_finish_collection();
            }
            else {
...
STATIC GC_bool GC_stopped_mark(GC_stop_func stop_func) {
    ...
    GC_COND_LOG_PRINTF("\n--> Marking for collection #%lu after %lu allocated "
                       "bytes\n",
                       (unsigned long)GC_gc_no + 1, (unsigned long)GC_bytes_allocd);
...
STATIC void GC_finish_collection(void) {
    ...
    GC_DBGLOG_PRINTF("GC #%lu freed %ld bytes, heap %lu KiB (" IF_USE_MUNMAP(
...
STATIC GC_bool GC_try_to_collect_general(GC_stop_func stop_func, GC_bool force_unmap) {
    GC_bool result;
...
#define INITIAL_HEAP_SECTS 32
#endif
        size_t new_capacity =
        GC_n_heap_sects > 0 ? (size_t)GC_n_heap_sects * 2 : INITIAL_HEAP_SECTS;
        ...
        if(GC_n_heap_sects > 0)
            BCOPY(old_heap_sects, new_heap_sects, GC_n_heap_sects * sizeof(struct HeapSect));
    ...
    if(old_capacity > 0) {
        ...
        GC_scratch_recycle_no_gww(old_heap_sects, old_capacity * sizeof(struct HeapSect));
#else
...
void GC_print_heap_sects(void) {
    ...
    for(i = 0; i < GC_n_heap_sects; i++) {
        ...
        GC_printf("Section %d from %p to %p %u/%lu blacklisted\n", i, (void*)start,
                  (void*)&start[len], nbl, (unsigned long)divHBLKSZ(len));
    }
}
#endif
...
GC_INNER GC_bool GC_expand_hp_inner(word n) {
    ...
    if(GC_max_heapsize != 0 &&
       (GC_max_heapsize < (word)bytes || GC_heapsize > GC_max_heapsize - (word)bytes)) {
    ...
    GC_INFOLOG_PRINTF("Grow heap to %lu KiB after %lu bytes allocated\n",
                      TO_KiB_UL(GC_heapsize + bytes), (unsigned long)GC_bytes_allocd);
    ...
    if((GC_last_heap_addr == 0 && !((word)space & SIGNB)) ||
       (GC_last_heap_addr != 0 && (word)GC_last_heap_addr < (word)space)) {
...
GC_INNER GC_bool GC_collect_or_expand(word needed_blocks, GC_bool ignore_off_page, GC_bool retry) {
    ...
    if(!GC_incremental && !GC_dont_gc &&
       ((GC_dont_expand && GC_bytes_allocd > 0) ||
        (GC_fo_entries > last_fo_entries && (last_bytes_finalized | GC_bytes_finalized) != 0 &&
         (GC_fo_entries - last_fo_entries) * GC_allocd_bytes_per_finalizer > GC_bytes_allocd) ||
        GC_should_collect())) {
        ...
        GC_bytes_allocd > 0 && (!GC_dont_expand || !retry) ? GC_default_stop_func :
                                                             GC_never_stop_func);
    ...
    blocks_to_get =
    (GC_heapsize - GC_heapsize_at_forced_unmap) / (HBLKSIZE * GC_free_space_divisor) + needed_blocks;
    ...
    if(!GC_expand_hp_inner(blocks_to_get) &&
       (blocks_to_get == needed_blocks || !GC_expand_hp_inner(needed_blocks))) {
