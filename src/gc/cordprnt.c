    ...
    for(CORD_set_pos(pos, format, 0); CORD_pos_valid(pos); CORD_next(pos)) {
        ...
        if(current == '%') {
            ...
            if(current == '%') {
                ...
                if(extract_conv_spec(pos, conv_spec, &width, &prec, &left_adj, &long_arg) < 0) {
                    return -1;
                }
                current = CORD_pos_fetch(pos);
                switch(current) {
                    ...
                    case 'r':
                        ...
                        if(width != NONE && len < (unsigned)width) {
                            char* blanks =
                            (char*)GC_MALLOC_ATOMIC((unsigned)width - len + 1);
...
!defined(__EMX__) /* and probably in other cases */
                    ...
                    switch(current) {
                        ...
                        case 'c':
                            ...
                            break;
                        case 's':
                        case 'p': (void)va_arg(args, char*); break;
                        case 'f':
                        case 'e':
                        case 'E':
                        case 'g':
                    ...
                    if(0 == res)
                        res = GC_VSNPRINTF(buf, max_size + 1, conv_spec, vsprintf_args);
